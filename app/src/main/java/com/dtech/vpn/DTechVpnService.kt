...
class DTechVpnService : VpnService() {
    ...
    private var v2rayController: libv2ray.CoreController? = null
        ...
        vpnThread = Thread {
            try {
                ...
                if (v2rayController == null) {
                    Libv2ray.initCoreEnv(filesDir.absolutePath, "default")
                    v2rayController = Libv2ray.newCoreController(object : libv2ray.CoreCallbackHandler {
                        override fun onEmitStatus(status: Long, message: String?): Long {
                            log("Xray Status: $status - $message")
                            return 0
                        }
